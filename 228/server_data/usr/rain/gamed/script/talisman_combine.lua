...
function EnchantTalismans(id, talisman1, cid, output, locked)
	...
	-- 法宝主属性二次强化灌魔
	if cid == 23860 or cid == 23861 or cid == 23862 or cid == 12021 or cid == 11784 or cid == 19716 then
...
function ZLuaTalisman_EnchantSkill(id, talisman1, cid, output, prof, enchantskilltype, enchantskillid, maxsameskillcol)
	...
	elseif enchantskilltype==2 then 
		...
		or talismanprof==99 
		or (talismanprof==21 and (prof==1 or prof==2 or prof==3 or prof==4 or prof==5))	--添加新职业需修改这行
		or (talismanprof==22 and (prof==7 or prof==8 or prof==9 or prof==10))						--添加新职业需修改这行
		then 
	...
	elseif (enchantskilltype==3 and prof==talismanskillprof)
			or (enchantskilltype==3 and prof==21 and talismanprof~=99 and talismanprof~=22 and talismanprof~=7 and talismanprof~=8 and talismanprof~=9 and talismanprof~=10)	--添加新职业需修改这行
			or (enchantskilltype==3 and prof==21 and talismanprof==99 and talismanskillprof~=7 and talismanskillprof~=8 and talismanskillprof~=9 and talismanskillprof~=10)		--添加新职业需修改这行
...
function ZLuaTalisman_ChooseEnchantSkill(prof,num,maxsamecol)
	...
	if     prof==99 then
		zchoice = {1,2,3,4,5,  7,8,9,10}	--添加新职业需修改这行
		zrand = math.ceil(math.random(table.getn(zchoice)))
		prof = zchoice[zrand]
	elseif prof==21 then
		zchoice = {1,2,3,4,5}	--添加新职业需修改这行
		zrand = math.ceil(math.random(table.getn(zchoice)))
		prof = zchoice[zrand]
	elseif prof==22 then
		zchoice = {7,8,9,10}	--添加新职业需修改这行
		zrand = math.ceil(math.random(table.getn(zchoice)))
		prof = zchoice[zrand]
	end
...
function ZLuaTalisman_GetProfInfo(id,SkillNoSeq)
	...
	for i,v in pairs(SkillsExLists) do
		for j=1,table.getn(SkillNoSeq) do 
			if i==SkillNoSeq[j] and (v.prof==1 or v.prof==2 or v.prof==3 or v.prof==4 or v.prof==5 or v.prof==7 or v.prof==8 or v.prof==9 or v.prof==10) then 		--添加新职业需修改这行
				prof_fromskill = v.prof
			end 
		end 
	end 

	if prof_fromid==1 or prof_fromid==2 or prof_fromid==3 or prof_fromid==4 or prof_fromid==5 or prof_fromid==7 or prof_fromid==8 or prof_fromid==9 or prof_fromid==10  		--添加新职业需修改这行
	then 
		prof_fromskill=prof_fromid
	end 
