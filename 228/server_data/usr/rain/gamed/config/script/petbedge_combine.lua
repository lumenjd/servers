...
function ZLuaPetBedgeSetSpirit(pet_tid,pet_in,pet_out,cid,aid,origin)  
	...
	if pet_in:QueryStar()<12 then 
		return 0,0,204	
	end 
	
	if pet_in:QueryData(DataIndex["skilllvl_9"])>=12 then 
		return 0,0,205
	else
		...
		pet_out:UpdateData(DataIndex["skilllvl_9"],pet_in:QueryData(DataIndex["skilllvl_9"])+1)
